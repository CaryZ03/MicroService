      ...
      <el-col :span="8" v-for="(order, index) in orders" :key="order.oid" class="order-col">
        <el-card :body-style="{ padding: '0px' }" class="order-card">
          ...
          <div class="card-footer">
            ...
            <el-button type="text" class="button"  @click="payOrder(order)" v-if="order.status === 'not paid'">付款</el-button>
            <el-button type="text" class="button"  @click="cancelOrder(order)" v-if="order.status === 'not paid' || order.status === 'paid'">取消订单</el-button>
            <el-button type="text" class="button"  @click="deleteOrder(order)" v-if="order.status === 'finished' || order.status === 'canceled'">删除订单</el-button>
            <el-button type="text" class="button"  @click="rateHotel(order)" v-if="(order.status === 'finished' || order.status === 'paid') && order.type === 'hotelOrder' && order.comment === false">评价酒店</el-button>
            <el-button type="text" class="button"  @click="updateRateHotel(order)" v-if="(order.status === 'finished' || order.status === 'paid') && order.type === 'hotelOrder' && order.comment === true">更新酒店评价</el-button>
            <el-button type="text" class="button"  @click="cancelRateHotel(order)" v-if="(order.status === 'finished' || order.status === 'paid') && order.type === 'hotelOrder' && order.comment === true">取消酒店评价</el-button>
          </div>
        </el-card>
      </el-col>
    </el-row>
...
import { Message } from 'element-ui';
  ...
  export default {
    ...
    methods: {
      ...
      async getOrders(){
        await this.axios.post(`/api/user/getUserOrder/${this.uid}`).then((resp) => {
      ...
      payOrder(order){
        ...
        if(order.status === "not paid"){
          this.axios.post(`/api/user/payOrder`,submitForm).then((resp) => {
      ...
      cancelOrder(order){
        ...
        if(order.status == "canceled"){
          ...
          this.axios.post(`/api/user/cancelOrder`,submitForm).then((resp) => {
            if(resp.data.success){
      ...
      async deleteOrder(order){
        ...
        await this.axios.post(`/api/user/deleteOrder`,submitForm).then(async (resp) => {
          let data = resp.data;
          if(data.success){
      ...
      async updateRateHotel(order){
        ...
        await this.axios.put(`/api/user/getHotelComment`,submitForm).then(async (resp) => {
          let data = resp.data;
          if(data.success){
      ...
      async cancelRateHotel(order){
        ...
        await this.axios.put(`/api/user/deleteHotelComment`,submitForm).then(async (resp) => {
          let data = resp.data;
          if(data.success){
      ...
      async confirmRateSubmit(){
        ...
        await this.axios.put(`/api/user/rateHotelByOid`,submitForm).then((resp) => {
          let data = resp.data;
          if(data.success){
...
.order-col {
  display: flex;
  flex: 0 0 33.3333%; /* Adjust this value if you want different sizing */
  max-width: 33.3333%; /* Adjust this value if you want different sizing */
