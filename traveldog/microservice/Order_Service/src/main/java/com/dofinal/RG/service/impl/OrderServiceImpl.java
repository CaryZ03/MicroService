...
import com.dofinal.RG.client.HotelClient;
import com.dofinal.RG.client.LocationClient;
import com.dofinal.RG.client.TrainClient;
...
public class OrderServiceImpl extends ServiceImpl<UserOrderMapper, UserOrder> implements OrderService {
    ...
    public List<UserOrderDemo> getUserOrderDemoByUid(String uid) {
        ...
        for (UserOrder userOrder : userOrderList) {
            ...
            switch (userOrder.getType()) {
                case "hotelOrder":
                    ...
                    hname = hotelClient.getHotelByHidMicro(hid).getName();
...
//                    }
                    ...
                    for (HotelCustomerOrder tmpHco : hcos) {
                        ...
                        cname = userClient.getCustomerByCid(cid).getName();
                ...
                case "trainOrder":
                    ...
                    Location departureLocation = locationClient.getLocationByLid(tco.getDepartLocationId());
                    Location arriveLocation = locationClient.getLocationByLid(tco.getArriveLocationId());
...
//                    }
                    ...
                    for (TrainCustomerOrder tmpTco : tcos) {
                        cname = userClient.getCustomerByCid(tmpTco.getCustomerId()).getName();
                ...
                case "mealOrder":
                    ...
                    for (MealCustomerOrder mco : mcos) {
                        cname = userClient.getCustomerByCid(mco.getCustomerId()).getName();
                        String mname = trainClient.getMealByMid(mco.getMealId()).getName();
    ...
    public NoticeRsp generateUserOrder(AddUserOrderReq req) {
        ...
        if (type.equals("hotelOrder")) {
            ...
            rsp.setMainName(hotelClient.getHotelByHidMicro(ho.getHid()).getName());
            ...
            rsp.setMainName(mo.getTid() + "火车的" + trainClient.getMealByMid(mo.getMid()).getName());
        ...
        for (Integer cid : cids) {
            String customerName = userClient.getCustomerByCid(cid).getName();
    ...
    public int generateHotelOrder(int oid, HotelOrderReq req, Timestamp curTime) {
        ...
        List<HotelRoom> hotelRooms = hotelClient.getHotelRoomByHidRidTime(hid, rid, curTime, fromTime, toTime);

        if (hotelRooms.size() < length) {
            return 0;
        } else {
            double price_rate = hotelClient.getHotelPriceRateByHid(hid);
            for (int i = 0; i < length; ++i) {
                // 获得HotelRoom
                HotelRoom hr = hotelRooms.get(i);
                double price = hotelClient.getRoomPriceByRid(rid) * price_rate;

                // 订购房间（数据库逻辑）
                byte[] bitmap = hr.getBitmap();
                bitmap = hotelClient.orderRoom(curTime, fromTime, toTime, bitmap);
                hr.setBitmap(bitmap);
                hotelClient.updateHotelRoom(hr);
    ...
    public int generateTrainOrder(int oid, TrainOrderReq req, Timestamp curTime) {
        ...
        int dlid = locationClient.getLidByProvinceCity(bl.getProvince(), bl.getCity());
        int alid = locationClient.getLidByProvinceCity(el.getProvince(), el.getCity());
        int bs = trainClient.getStationSequenceByTidAndLid(tid, dlid);
        int as = trainClient.getStationSequenceByTidAndLid(tid, alid);
        Timestamp depature_time = trainClient.getTrainArrivalTimeByTidLid(tid, dlid);
        Timestamp arrive_time = trainClient.getTrainArrivalTimeByTidLid(tid, alid);

        int count = trainClient.getSeatCountByTidAndStationIndex(tid, bs, as, type);
        if (count < length) {
            return 0;
        } else {
            double price_rate = trainClient.findTrainPriceRateByTid(tid);
            for (int i = 0; i < length; ++i) {
                // 进行座位的购买（数据库的逻辑）
                String sid = trainClient.buySeatByTidAndStationIndex(tid, bs, as, type);
                double price = trainClient.getPriceBySeatType(type) * price_rate;
                // 生成订单
    ...
    public int generateMealOrder(int oid, MealOrderReq req, Timestamp curTime) {
        ...
        int count = trainClient.getTrainMealCountByTidAndMid(tid, mid);

        if (count < length) {
            return 0;
        } else {
            Meal meal = trainClient.getMealByMid(mid);
            double price_rate = trainClient.findTrainPriceRateByTid(tid);
            for (int i = 0; i < length; ++i) {
                // 购买火车餐（数据库逻辑）
                trainClient.buyMeal(tid, mid);
                MealCustomerOrder mco = orders.get(i);
                double price = trainClient.getMealPriceByMid(mid) * price_rate;
    ...
    public NoticeRsp cancelUserOrder(OtherUserOrderReq req) {
        ...
        if (type.equals("trainOrder")) {
            ...
            rsp.setMainName(trainClient.getMealByMid(orders.get(0).getMealId()).getName());
    ...
    public int rateHotel(HotelCommentReq req) {
        ...
        Hotel hotel = hotelClient.getHotelByHidMicro(hid);
        if (hotel == null)
            return 0;

        HotelComment hotelComment = new HotelComment(-1,oid, hid, req.getRate(), req.getComment());
    ...
    public int cancelTrainOrder(List<TrainCustomerOrder> orders) {
        for (TrainCustomerOrder order : orders) {
            ...
            trainClient.cancelSeatByTidAndStationIndex(tid, sid, bs, as);
        }
        return 1;
    }
    ...
    public int cancelMealOrder(List<MealCustomerOrder> orders) {
        for (MealCustomerOrder order : orders) {
            trainClient.cancelMeal(order.getTrainId(), order.getMealId());
        }
        return 1;
    }
    ...
    public int cancelHotelOrder(List<HotelCustomerOrder> orders) {
        for (HotelCustomerOrder order : orders) {
            ...
            HotelRoom hotelRoom = hotelClient.getHotelRoomByHidRidRNumber(hid, rid, rNumber);

            byte[] curBinary = hotelRoom.getBitmap();
            curBinary = hotelClient.cancelRoom(curTime, startTime, endTime, curBinary);
            hotelRoom.setBitmap(curBinary);
            hotelClient.updateHotelRoom(hotelRoom);
        }
        return 1;
    }
    ...
    @Override
    public UserOrder getUserOrderByOid(Integer oid){
        return userOrderMapper.getUserOrderByOid(oid);
    }

    @Override
    public List<Integer> getOrderIdsByTid(String tid){
        return trainOrderMapper.getOrderIdsByTid(tid);
    }

    @Override
    public List<TrainCustomerOrder> findTrainOrderByOid(Integer oid){
        return trainOrderMapper.findTrainOrderByOid(oid);
    }

    @Override
    public List<TrainCustomerOrder> findTrainOrderByTid(String tid){
        return trainOrderMapper.findTrainOrderByTid(tid);
    }

    @Override
    public List<HotelCustomerOrder> findHotelOrderByHid(Integer hid){
        return hotelOrderMapper.findHotelOrderByHid(hid);
    }

    @Override
    public List<HotelCustomerOrder> findHotelOrderByOid(Integer oid){
        return hotelOrderMapper.findHotelOrderByOid(oid);
    }

    @Override
    public List<Integer> getOidByUid(String uid){
        return userOrderMapper.getOidByUid(uid);
    }
    @Override
    public Integer deleteHotelOderByOid(Integer oid){
        return hotelOrderMapper.deleteHotelOderByOid(oid);
    }

    @Override
    public Integer deleteMealOrderByOid(Integer oid){
        return mealOrderMapper.deleteMealOrderByOid(oid);
    }

    @Override
    public Integer deleteTrainOrderByOid(Integer oid){
        return trainOrderMapper.deleteTrainOrderByOid(oid);
    }

    @Override
    public Integer deleteUserOrderByUid(String uid){
        return userOrderMapper.deleteUserOrderByUid(uid);
    }

    @Override
    public List<HotelComment> getHotelCommentByHid(@PathVariable int hid){
        return hotelCommentMapper.getHotelCommentByHid(hid);
    }

    @Override
    public List<UserOrder> getUserOrderByUid(String uid){
        return userOrderMapper.getUserOrderByUid(uid);
    }
}