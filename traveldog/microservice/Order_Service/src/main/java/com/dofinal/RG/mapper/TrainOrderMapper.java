...
public interface TrainOrderMapper extends BaseMapper<TrainCustomerOrder> {
    @Select("select * from trains_orders where o_id = #{oid}")
    ...
    @Select("select * from trains_orders where c_id = #{cid}")
    List<TrainCustomerOrder> findTrainOrderByCid(int cid);
    @Select("select * from trains_orders where o_id = #{oid} and c_id = #{cid}")
