...
public class RoomController {
    ...
    public BaseRsp<List<HotelRoom>> getHotelRoomByHid(@PathVariable("hid") int hid) {
    ...
    public BaseRsp<List<HotelRoom>> getHotelRoomByTime(@RequestBody RoomReq req) {
    ...
    public BaseRsp<List<HotelRoom>> getHotelRoomByHidAndTime(@RequestBody RoomReq req) {
        Timestamp curTime = new Timestamp(System.currentTimeMillis());
        BaseRsp<List<HotelRoom>> rsp = new BaseRsp<>();
    ...
    public BaseRsp<List<HotelRoom>> getHotelRoomByRidAndTime(@RequestBody RoomReq req) {
        Timestamp curTime = new Timestamp(System.currentTimeMillis());
        BaseRsp<List<HotelRoom>> rsp = new BaseRsp<>();
    ...
    Integer updateHotelRoom(@RequestBody  HotelRoom hr) {
        return roomService.updateHotelRoom(hr);
    }

    @GetMapping("/room/getPriceByRid/{rid}")
    Double getRoomPriceByRid(@PathVariable int rid) {
        return roomService.getRoomPriceByRid(rid);
    }

    @PostMapping("/orderRoom/{curTime}/{startTime}/{endTime}")
    public byte[] orderRoom(@PathVariable Timestamp curTime, @PathVariable Timestamp startTime,
                            @PathVariable Timestamp endTime, @RequestBody byte[] curBinary) {
        return roomService.orderRoom(curTime, startTime, endTime, curBinary);
    }

    @PostMapping("/cancelRoom/{curTime}/{startTime}/{endTime}")
    public byte[] cancelRoom(@PathVariable Timestamp curTime, @PathVariable Timestamp startTime,
                             @PathVariable Timestamp endTime, @RequestBody byte[] curBinary) {
        return roomService.cancelRoom(curTime, startTime, endTime, curBinary);
    }

    @GetMapping("/hotel/getRoomByHidRidRNumber/{hid}/{rid}/{rNumber}")
    HotelRoom getHotelRoomByHidRidRNumber(@PathVariable int hid, @PathVariable int rid,
                                          @PathVariable String rNumber) {
        return roomService.findHotelRoomByHidRidRNumber(hid, rid, rNumber);
    ...
    List<HotelRoom> getHotelRoomByHidRidTime(@PathVariable int hid, @PathVariable int rid, @PathVariable Timestamp curTime,
        ...
        return roomService.findHotelRoomByHidRidTime(hid,rid,curTime,startTime,endTime);
