...
public interface HotelMapper extends BaseMapper<Hotel> {
        @Select("select h_id as id, h_name as name, l_id as locationId, price_rate as priceRate, h_tel as phoneNumber, detail_location as detailLocation, h_description as description from hotels where l_id = #{lid}")
        List<Hotel> findHotelByLid(int lid);

        @Select("select * from hotels")
        List<Hotel> findHotels();

        @Select("select * from hotels where h_id = #{hid}")
        Hotel getHotelByHid(int hid);

        @Select("select price_rate from hotels where h_id = #{hid}")
        Double findHotelPriceRateByHid(int hid);

        @Insert("insert into hotels(h_id, h_name, l_id, price_rate, h_tel, detail_location, h_description) values" +
        ...
        @Update("update hotels set h_name = #{hot.name}, l_id = #{hot.locationId}, detail_location = #{hot.detailLocation}, "
                        +
        ...
        @Delete("delete from hotels where h_id = #{hid}")
