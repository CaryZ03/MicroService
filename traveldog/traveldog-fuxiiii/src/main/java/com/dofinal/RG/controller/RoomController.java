...
public class RoomController {
    ...
    public BaseRsp<List<HotelRoom>> findHotelRoomByHidAndTime(@RequestBody RoomReq req) {
        Timestamp curTime = new Timestamp(System.currentTimeMillis());
        BaseRsp<List<HotelRoom>> rsp = new BaseRsp<>();
        List<HotelRoom> hotelRooms = roomService.findHotelRoomByHidAndTime(req.getHid(), curTime, req.getFromTime(), req.getToTime());
        if (hotelRooms != null) {
            rsp.setSuccess(true);
    ...
    public BaseRsp<List<HotelRoom>> findHotelRoomByRidAndTime(@RequestBody RoomReq req) {
        Timestamp curTime = new Timestamp(System.currentTimeMillis());
        BaseRsp<List<HotelRoom>> rsp = new BaseRsp<>();
        List<HotelRoom> hotelRooms = roomService.findHotelRoomByRidAndTime(req.getRid(), curTime, req.getFromTime(), req.getToTime());
        if (hotelRooms != null) {
            rsp.setSuccess(true);
    ...
    public BaseRsp orderRoom(@RequestBody RoomReq req) {
        ...
        if (roomService.checkHotelRoom(hotelRoom, curTime, startTime, endTime)) {
            ...
            String content = "您已成功预定" + hotelService.findHotelById(req.getHid()).getName() + "酒店的" + req.getrNumber() + "号房间";
    ...
    public BaseRsp cancelRoom(@RequestBody RoomReq req) {
        ...
        if (roomService.updateHotelRoom(hotelRoom) != 0) {
            ...
            String content = "您已成功取消" + hotelService.findHotelById(req.getHid()).getName() + "酒店的" + req.getrNumber() + "号房间的订单";
            noticeUtil.addNotice(req.getUid(), content);
        } else {
            rsp.setSuccess(false);
            rsp.setMessage("cancel room false!");
            String content = hotelService.findHotelById(req.getHid()).getName() + "酒店的" + req.getrNumber() + "号房间订单取消失败。";
            noticeUtil.addNotice(req.getUid(), content);
        }
        return rsp;
    }
