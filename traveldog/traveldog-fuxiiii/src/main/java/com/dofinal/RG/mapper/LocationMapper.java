...
public interface LocationMapper extends BaseMapper<LocationMapper> {
    ...
    //模糊查询
    @Select("select l_id, province, city from locations where province like concat('%',#{province},'%')")
    List<Location> getLocationByProvince(String province);

    //模糊查询
    @Select("select distinct l_id from locations where province like concat('%',#{province},'%')")
