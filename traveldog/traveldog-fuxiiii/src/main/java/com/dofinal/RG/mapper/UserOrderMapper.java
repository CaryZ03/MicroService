...
public interface UserOrderMapper extends BaseMapper<UserOrder> {
    ...
    @Select("select o_id as orderId, u_id as userId, price, add_time as addTime, uo_status as status, " + "service_type as type, is_comment as isComment from orders where u_id = #{uid}")
    List<UserOrder> getUserOrderByUid(String uid);

    @Select("select o_id as orderId, u_id as userId, price, add_time as addTime, uo_status as status, " + "service_type as type from orders where o_id = #{oid}")
    UserOrder getUserOrderByOid(int oid);

    @Select("select o_id as orderId, u_id as userId, price, add_time as addTime, uo_status as status, + " + "service_type as type from orders where u_id = #{uid} and add_time = #{addTime}")
    UserOrder getUserOrderByUidAddTime(@Param("uid") String uid, @Param("addTime") String addTime);

    @Select("select o_id as orderId, u_id as userId, price, add_time as addTime, uo_status as status, " + " service_type as type from orders")
    ...
    @Insert("insert into orders(u_id, price, add_time, uo_status, service_type) values" + "(#{uo.userId},#{uo.price},#{uo.addTime},#{uo.status},#{uo.type})")
