...
public interface UserMapper extends BaseMapper<User> {
    ...
    @Update("update traveldog.users set u_name = #{user.name}, u_password = #{user.password}, u_tel = " + "#{user.phoneNumber}, u_money = #{user.money}, u_status = #{user.status} where u_id = #{user.id}")
    Integer updateUser(@Param("user") User user);

    @Insert("insert into traveldog.users(u_id, u_name, u_password, u_tel, u_money, u_status) " + " values (#{user.id}, #{user.name}, #{user.password}, #{user.phoneNumber}, #{user.money}, #{user.status})")
