...
public interface CustomerMapper extends BaseMapper<Customer> {
    ...
    @Insert("insert into traveldog.customers(c_name, id_card, c_tel, c_age) values" + "(#{cus.name},#{cus.idCard},#{cus.phoneNumber},#{cus.age})")
    Integer insertCustomer(@Param("cus") Customer cus);

    @Update("update traveldog.customers set c_name = #{cus.name}, c_tel = #{cus.phoneNumber}, " + "c_age = #{cus.age}, c_name = #{cus.name}" + " where c_id = #{cus.id}")
