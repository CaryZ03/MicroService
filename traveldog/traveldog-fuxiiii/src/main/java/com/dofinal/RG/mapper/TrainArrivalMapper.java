...
public interface TrainArrivalMapper extends BaseMapper<TrainArrival> {
    ...
    @Select("select t_id from train_arrivals as ta1 where ta1.l_id = #{sl} and ta1.arrivalTime > #{curTime} and date(date_add(#{curTime}, interval 1 day)) > date(ta1.arrivalTime)  " + "and exists(select * from train_arrivals as ta2 where ta2.l_id = #{el} and ta2.t_id = ta1.t_id and ta1.station_sequence < ta2.station_sequence)")
    List<String> getTidByLidAndTimes(@Param(value = "curTime") String curTime, @Param(value = "sl") Integer sl, @Param(value = "el") Integer el);
