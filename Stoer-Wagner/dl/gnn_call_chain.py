...
# Load the call matrix (assuming it's already loaded as a numpy array)
call_matrix = np.load("../results/dependence/call_count.npy")
names = [
    {
        "func_name": "Config",
...
# Define the GNN model (simple graph autoencoder)
...
input_dim = call_matrix.shape[0]
model = GraphEncoder(input_dim=input_dim, hidden_dim=64, embed_dim=16)
...
for epoch in range(100):
    ...
    loss = torch.norm(adj_matrix - embeddings @ embeddings.T)  # Reconstruction loss
    optimizer.zero_grad()
    loss.backward()
    optimizer.step()
