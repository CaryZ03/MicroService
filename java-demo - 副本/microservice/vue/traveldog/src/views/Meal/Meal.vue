        ...
        <div class="searchMeal" v-show="!searchFlag">
            <div class="search-container">
                ...
                <div class="search-area">
                    <!--搜索区域-->
                    <div class="header">
                        <el-input v-model="searchTid" placeholder="所要查询火车餐的列车编号" clearable @clear="resetSearch()"
                    ...
                    <!--表格区域展示视图数据-->
                    <div class="wrapper">
                        <el-table :data="mealsPaginatedData" style="width: 100%"
                            ...
                            <el-table-column prop="price" label="火车餐价格" width="250" :formatter="formatPrice"
...
export default {
    ...
    methods: {
        ...
        async getTidList() {
            try {
                const resp = await this.axios.get(`/train-service-api-8083/train/getTidByUid/${this.uid}`);
                let data = resp.data;
        ...
        async getMeals() {
            ...
            for (let i = 0; i < this.tidList.length; i++) {
                ...
                let promise = this.axios.post(`/train-service-api-8083/meal/getTrainMeals`, submitForm).then((resp) => {
                    let data = resp.data;
                    // 合并content数组
        ...
        async buyMeal(meal) {
            ...
            await this.axios.post(`/user-service-api-8084/customer/getCidByTidStrict/${meal.tid}`).then(async (resp) => {
                let data = resp.data;
                let allContent = []; // 用于收集所有的content
                for (let i = 0; i < data.content.length; i++) {
                    await this.axios.get(`/user-service-api-8084/customer/getByCid/${data.content[i]}`).then((resp) => {
                        let data = resp.data;
        ...
        async customersselectionSubmit() {
            ...
            if (this.selectedCustomers === null || this.selectedCustomers.length === 0) {
                ...
                await this.axios.post(`/order-service-api-8082/user/addOrder`, submitForm).then((resp) => {
        ...
        async searchMeals() {
            ...
            await this.axios.post(`/train-service-api-8083/meal/getTrainMeals`, submitForm).then((resp) => {
                //成功获取
                let data = resp.data;
