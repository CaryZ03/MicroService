...
public class MealController {
    ...
    public boolean buyMealByTidMid(@PathVariable String tid, @PathVariable int mid){
        return mealService.buyMeal(tid, mid);
    }

    @PostMapping("meal/cancelByTidMid/{tid}/{mid}")
    public boolean cancelMealByTidMid(@PathVariable String tid, @PathVariable int mid){
        return mealService.cancelMeal(tid, mid);
    }

    @GetMapping("/meal/getCountByTidMid/{tid}/{mid}")
    int getTrainMealCountByTidAndMid(@PathVariable String tid, @PathVariable int mid){
        return mealService.getTrainMealCountByTidAndMid(tid, mid);
    }

    @GetMapping("/meal/getByMid/{mid}")
    Meal getMealByMid(@PathVariable int mid){
        return mealService.getMealByMid(mid);
    }

    @GetMapping("/meal/getPriceByMid/{mid}")
    double getMealPriceByMid(@PathVariable int mid){
        return mealService.getMealPriceByMid(mid);
    }
