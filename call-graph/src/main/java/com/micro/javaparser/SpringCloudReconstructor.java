...
public class SpringCloudReconstructor {
    ...
    public static void main(String[] args) {
        String projectRoot = "D:\\Projects\\MicroService\\call-graph\\microservices";
        File projectDir = new File(projectRoot);
        List<File> microServices = new ArrayList<>(Arrays.asList(projectDir.listFiles()));
        System.out.println(microServices);

        JavaParser parser = new JavaParser();

        Map<String, List<String>> splits = new HashMap<>();
        String filePath = "microservices2.json";
        try (FileReader reader = new FileReader(filePath)) {
            Gson gson = new Gson();
            Type type = new TypeToken<Map<String, List<String>>>() {}.getType();
            splits = gson.fromJson(reader, type);
        } catch (IOException e) {
            e.printStackTrace();
        }

        for (File ms : microServices) {
            String fileIndex = ms.getName().split("microservice")[1];
            List<String> nodes = splits.get(fileIndex);
            System.out.println(nodes);

            String microServiceRoot = ms.getAbsolutePath() + "\\src\\main\\java";
...
//            parseDirectory(ms, fileList);
//            for (File file : fileList) {
//
//            }
//
//            String filename = Paths.get(projectRoot).relativize(Paths.get(ms.getAbsolutePath())).toString();
//            try (FileInputStream in = new FileInputStream(ms)) {
