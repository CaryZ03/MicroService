...
<template>
    <div class="Tickets">
            ...
            <div class="container">
                ...
                <el-form @submit.prevent="submitForm">
                    ...
                    <el-form-item label="出发日期">
                        <el-date-picker v-model="date" type="datetime" placeholder="选择日期和时间"
                            :picker-options="pickerOptions" @change="handleDateChange" required></el-date-picker>
                    </el-form-item>
                    <el-form-item>
        ...
        <div class="tickets" v-show="searchFlag">
            ...
            <!--2、表格区域展示视图数据-->
            <div class="wrapper">
                <el-table :data="trainsPaginatedData" border style="width: 100%"
                    :default-sort="{ prop: 'departure_time', order: 'descending' }">
                    ...
                    <el-table-column prop="arrive_time" label="到达时间" width="180" sortable
                        :sort-method="sortDestinationTime">
                        <template slot-scope="scope">{{ getDestinationTime(scope.row.locations) }}</template>
                    </el-table-column>
                    <el-table-column prop="duration_time" label="旅途时间" width="180" sortable
                        :sort-method="sortDurationTime">
            ...
            <el-dialog style="width: 100%;height: 1000px;" title="选择为哪位顾客购买车票" :visible.sync="customersselectionDialog">
                <div class="dialog-body">
                    <el-table :data="customersTableData" border style="width: 100%"
...
export default {
    ...
    methods: {
        ...
        async search() {
            ...
            try {
                let resp = await this.axios.post(`/train-service-api-8083/train/getByTimeAndLocation`, submitForm);
                let data = resp.data;
        ...
        selectSeat(seat) {
            ...
            this.axios.get(`/user-service-api-8084/customer/getByUid/${this.uid}`).then((resp) => {
                let data = resp.data;
        ...
        async getCustomersAlreadyBuyTickets(tid) {
            try {
                let resp = await this.axios.post(`/user-service-api-8084/customer/getCidByTid/${tid}`);
                let data = resp.data;
        ...
        async customersselectionSubmit() {
            ...
            console.log("length of selected: ", this.selectedCustomers.length);
            if (this.selectedCustomers === null || this.selectedCustomers.length === 0) {
                ...
                if (errorCids.length > 0) {
                    ...
                    try {
                        let resp = await this.axios.post(`/order-service-api-8082/user/addOrder`, submitForm);
                        let data = resp.data;
                        if (data.success) {
