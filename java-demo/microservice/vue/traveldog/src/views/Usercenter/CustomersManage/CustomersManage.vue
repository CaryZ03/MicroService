<template>
    <div class="tickets">
        <!--搜索区域-->
        <div class="header">
            <el-input v-model="query" placeholder="顾客姓名" clearable @clear="resetSearch()"
                @blur="handleInputBlur"></el-input><!--blur设置一个监听-->
...
export default {
    ...
    methods: {
        ...
        //列表获取
        http(uid) {
            this.axios.get(`/user-service-api-8084/customer/getByUid/${uid}`).then((resp) => {
                let data = resp.data;
        ...
        submitAdd() {
            if (!this.form.cName || !this.form.idCard || !this.form.cAge || !this.form.cTel) {
                ...
                if (this.submitType == "add") {
                    this.axios.post(`/user-service-api-8084/user/addCustomer`, this.form).then((resp) => {
                        let data = resp.data;
                        if (data.success) {
        ...
        handleDelete(index, row) {
            this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
                ...
                this.axios.post('/user-service-api-8084/user/deleteCustomer', this.form).then((resp) => {
                    //如果删除成功
