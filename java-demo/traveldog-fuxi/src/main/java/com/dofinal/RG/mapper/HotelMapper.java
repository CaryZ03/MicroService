...
public interface HotelMapper extends BaseMapper<Hotel> {
    ...
    @Insert("insert into traveldog.hotels(h_id, h_name, l_id, price_rate, h_tel, detail_location, h_description) values" + "(#{hot.id}, #{hot.name}, #{hot.locationId}, #{hot.priceRate}, #{hot.phoneNumber},#{hot.detailLocation}, #{hot.description})")
    Integer insertHotel(@Param("hot") Hotel hot);

    @Update("update traveldog.hotels set h_name = #{hot.name}, l_id = #{hot.locationId}, detail_location = #{hot.detailLocation}, " + " h_tel = #{hot.phoneNumber}, price_rate = #{hot.priceRate}, h_description = #{hot.description}" + " where h_id = #{hot.id}")
